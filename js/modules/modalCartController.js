  ...
  if (cartControl.cartData.length) {
    ...
    const cardsData = await Promise.all(
      cartControl.cartData.map(
        async (item) =>
          await getData(
            `https://scented-tremendous-discovery.glitch.me/api/products/${item.id}`,
          ),
      ),
    );
...
const delCaratItem = (e) => { /* удаляем из корзины */
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch(
      'https://scented-tremendous-discovery.glitch.me/api/orders/', 
    {
      method: 'POST',
      headers: {
    ...
    cartForm.innerHTML = `
      <h3>Заказ оформлен, номера заказа ${order.orderId}</h3>
      `;
...
export const modalCartController = () => {
  renderCartList();
  cartListElem.addEventListener('click', delCaratItem);
